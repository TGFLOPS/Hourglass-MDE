...
class SwinTransformerV2(nn.Module):
    ...
    def forward(self, x):
        """Forward function."""
        # print("before_patch_embedding : ", x.shape)
        x = self.patch_embed(x)
        # print("after_patch_embedding : ", x.shape)
        ...
        for i in range(self.num_layers):
            layer = self.layers[i]
            # print("before {0}th layer shape : {1} | Wh: {2}, Ww: {3}".format(i, x.shape, Wh, Ww))
            x_out, H, W, x, Wh, Ww = layer(x, Wh, Ww)
            # print("after {0}th layer : {1}".format(i, x_out.shape))
            ...
            if i in self.out_indices:
                ...
                # print("{0}th layer shape : {1}".format(i, out.shape))
                outs.append(out)
